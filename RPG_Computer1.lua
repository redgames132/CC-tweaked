-- =======================================================
-- PILGRAMO - SISTEMA DUAL PC (REDNET)
-- =======================================================
local mon = peripheral.find("monitor")
local speaker = peripheral.find("speaker") -- Para os efeitos sonoros das espadas

-- Liga a placa de rede para conversar com a Caixa de Som
...
local function centralizar(y, texto, corTexto, corFundo)
    ...
    mon.setCursorPos(math.max(1, x), y); mon.setTextColor(corTexto or colors.white)
    if corFundo then mon.setBackgroundColor(corFundo) end; mon.write(texto)
end
...
local function turnoInimigo()
    if inimigoAtual and inimigoAtual.hp > 0 then
        if math.random(1, 100) <= 12 then
            mensagemLog = inimigoAtual.nome .. " ERROU!"; tocar("entity.player.attack.nodamage", 1); return
        end
        ...
        mensagemLog = inimigoAtual.nome .. " causou " .. danoReal .. " dano!"; tocar("entity.player.hurt", 1)
        if jogador.hp <= 0 then jogador.hp=0; ESTADO="MENU"; fs.delete(ARQUIVO_SAVE); tocar("entity.wither.death", 0.5) end
    end
end
...
local function vitoria()
    ...
    if jogador.xp >= jogador.level * 20 then
        ...
        mensagemLog="LEVEL UP!"; tocar("entity.player.levelup", 1); atualizarTela(); os.sleep(1.5)
    end
...
local function loopJogo()
    ...
    -- Manda a caixa de som começar a tocar a música!
    rednet.broadcast("TOCA_MUSICA")
    ...
    while rodando do
        ...
        if ESTADO == "MENU" then
            ...
            if temSave and y>=7 and y<=9 then carregarJogo(); ESTADO="MAPA"; atualizarTela()
            elseif (not temSave and y>=9 and y<=11) or (temSave and y>=11 and y<=15) then
                jogador={hp=60, maxHp=60, tp=0, level=1, xp=0, ouro=10, pocoes=3, pocoesMax=0, danoExtra=0, defesa=0, magiaExtra=0, zona=1, nodo=1}; ESTADO="MAPA"; salvarJogo(); atualizarTela()
            end
        elseif ESTADO == "MAPA" then
            if y>=12 and y<=14 then
                if x<=larg/2 then
                    ...
                    if jogador.nodo == 6 then
                        ...
                        mensagemLog="CUIDADO CHEFE!"; tocar("entity.ender_dragon.growl", 1.0); ESTADO="BATALHA"
                    else
                        if math.random(1, 100)<=35 then
                            eventoAtual=listaEventos[math.random(1, #listaEventos)]; eventoAtual.acao(); ESTADO="EVENTO"
                        else
                            local mz = 1.0 + ((jogador.zona - 1) * 0.20)
                            ...
                            mensagemLog=inimigoAtual.nome.." ataca!"; ESTADO="BATALHA"
                        end
                    end
                    atualizarTela()
                else ESTADO="LOJA"; atualizarTela() end
            end
        elseif ESTADO == "EVENTO" then
            if y>=13 and y<=15 then jogador.nodo=jogador.nodo+1; if jogador.nodo>6 then jogador.nodo=6 end; ESTADO="MAPA"; salvarJogo(); atualizarTela() end
        elseif ESTADO == "LOJA" then
            if y>=5 and y<=7 then
                if x<=28 and jogador.ouro>=10 then jogador.ouro=jogador.ouro-10; jogador.pocoes=jogador.pocoes+1; tocar("entity.experience_orb.pickup",1)
                elseif x>28 and jogador.ouro>=25 then jogador.ouro=jogador.ouro-25; jogador.pocoesMax=jogador.pocoesMax+1; tocar("entity.experience_orb.pickup",1) end
            elseif y>=9 and y<=11 then
                if x<=28 and jogador.ouro>=35 then jogador.ouro=jogador.ouro-35; jogador.danoExtra=jogador.danoExtra+5; tocar("item.armor.equip_iron",1)
                elseif x>28 and jogador.ouro>=35 then jogador.ouro=jogador.ouro-35; jogador.defesa=jogador.defesa+2; tocar("item.shield.equip",1) end
            elseif y>=13 and y<=15 then
                if x<=28 and jogador.ouro>=45 then jogador.ouro=jogador.ouro-45; jogador.maxHp=jogador.maxHp+20; jogador.hp=jogador.hp+20; tocar("item.armor.equip_diamond",1)
                elseif x>28 and jogador.ouro>=55 then jogador.ouro=jogador.ouro-55; jogador.magiaExtra=jogador.magiaExtra+10; tocar("block.amethyst_block.chime",1) end
            elseif y>=17 and y<=19 then salvarJogo(); ESTADO="MAPA" end
            atualizarTela()
        elseif ESTADO == "BATALHA" or ESTADO == "SUBMENU_MAGIA" or ESTADO == "SUBMENU_ITEM" then
            if y>=16 and y<=18 then
                if ESTADO == "BATALHA" then
                    if x>=2 and x<=14 then
                        tocar("entity.player.attack.sweep", 1.2); jogador.tp=math.min(100, jogador.tp+15)
                        local mult=1; if math.random(1,100)<=20 then mult=2; tocar("entity.player.attack.crit", 1); mensagemLog="CRITICO! " else mensagemLog="" end
                    ...
                    elseif x>=44 and x<=56 then
                        if inimigoAtual.isBoss then mensagemLog="Nao fuja!"; tocar("entity.villager.no",1); atualizarTela(); os.sleep(1); turnoInimigo(); atualizarTela()
                        else
                            if math.random(1,100)<=75 then mensagemLog="Fugiu!"; atualizarTela(); os.sleep(1); ESTADO="MAPA"; atualizarTela()
                            else mensagemLog="Falhou!"; atualizarTela(); os.sleep(1); turnoInimigo(); atualizarTela() end
                        end
                    end
                elseif ESTADO == "SUBMENU_MAGIA" then
                    if x<=22 and jogador.tp>=40 then
                        jogador.tp=jogador.tp-40; jogador.hp=math.min(jogador.maxHp, jogador.hp+40); mensagemLog="Curou! (+40)"; tocar("entity.generic.drink",1)
                    ...
                    elseif x>22 and x<=44 and jogador.tp>=50 then
                        ...
                        tocar("entity.lightning_bolt.thunder",1); ESTADO="BATALHA"; atualizarTela(); os.sleep(1); if inimigoAtual.hp<=0 then vitoria() else turnoInimigo(); atualizarTela() end
                    elseif x>44 then ESTADO="BATALHA"; atualizarTela() end
                elseif ESTADO == "SUBMENU_ITEM" then
                    if x<=26 then
                        if jogador.pocoes>0 or jogador.pocoesMax>0 then
                            ...
                            mensagemLog="Usou pocao!"; tocar("entity.generic.drink",1); ESTADO="BATALHA"; atualizarTela(); os.sleep(1); turnoInimigo(); atualizarTela()
                        end
                    elseif x>26 then ESTADO="BATALHA"; atualizarTela() end
                end
            end
        end
    end
...
local function escutarSaida()
    while rodando do
        ...
        if p1 == keys.q then
            rodando = false
            mon.setBackgroundColor(colors.black); mon.clear()
            -- Manda a caixa de som parar de tocar quando você sai do jogo
            rednet.broadcast("PARA_MUSICA")
        end
    end
end

local sucesso, erro = pcall(function() parallel.waitForAny(escutarSaida, loopJogo) end)
